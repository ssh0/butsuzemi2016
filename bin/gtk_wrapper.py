...
class interactive(Gtk.Window):
    ...
    def __init__(self, func, kwargs, title='!nteractive', show_status=False):
    ...
    def entry(self, kw, arg):

        def entry_interact(entry, *args):
            self.kwargs[kw] = str(entry.get_text())
            self.status_change()

        # create entry widget in listbox
        row = Gtk.ListBoxRow()
        hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=50)
        row.add(hbox)
        ...
        entry = Gtk.Entry()
        entry.set_text(self.set_value(kw, arg[0]))
        entry.connect('focus-out-event', entry_interact)
    ...
    def combobox_tuple(self, kw, arg):
        ...
        argstore = Gtk.ListStore(str)
        keys = [k for k, v in arg]
        values = [v for k, v in arg]
        for a in keys:
            argstore.append([a])
    ...
    def button(self, kw, arg):

        row = Gtk.ListBoxRow()
        hbox = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=50)
        row.add(hbox)
        button = Gtk.Button(kw)
        button.connect('clicked', arg)
...
if __name__ == '__main__':
    ...
    def f(x=12, y=20, z='foo', o=False, i=20):
